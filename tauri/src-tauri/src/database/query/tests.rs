...
async fn test_clear_unreferenced_assets_in_trashbox() {
    ...
    sqlx::query!("insert into assets (hash, data) values ('1', x'00');")
        .execute(&mut *tx)
        .await
        .unwrap();

    o.deleted = SqliteBool(true);

    clear_unreferenced_deleted_assets(&mut tx).await.unwrap();

    tx.commit().await.unwrap();

    let r = sqlx::query_scalar!("select count(*) from assets;")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(r, 1);
...
async fn test_fetch_deleted_outline_trees() {
    ...
    let mut tree = Outline::create_tree(2, 2);
    tree[0].deleted = SqliteBool(true);
